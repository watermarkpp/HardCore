...
源码提交1ea58045及8b32fb15完成拾取居中、Buff左对齐、月魔蜘蛛168魔法盾伤害、统一近战1.5GU、安全区丢目标和敌方锁定施法拒绝、空安全区快路。爬行怪黄圈半径仅+30%，9个长体型身份按八方向偏移，S保持，站立怪保持原样；用户已确认最新四张预览满意，不实施贴图中心自动对齐方案。脚点、碰撞与贴图不随黄圈调整。账本与验证见 `docs/repair_20260923_six/REQUEST_LEDGER.md`、`VERIFICATION.md`；综合校准器玩家走路断言在旧基线和本树均FAIL，怪物黄圈专项独立验证。APK与设备测试NOT_RUN，版本未改。

## 2026-09-23：怪物召唤与祖玛教主之家单图发布

幻影蜘蛛182改召爆裂蜘蛛183；非 Boss 敌方召唤源按 spawn_slot 硬限5（存活+预留），Boss保留配置上限。祖玛教主160按用户追加裁决每次4～7、上限15，恢复原版四种普通召唤池156/153/150/128，不召159精英。空掉落与刷新准入分离，仅解除33/183/241因空掉落产生的阻塞，不改掉落槽/属性。用户最新保存的祖玛教主之家已精确发布为4普通+5精英+1教主，其余66图发布记录不变。验证、失败分类与提交状态见 `docs/repair_20260923_summons/VERIFICATION.md`。后续六项UI/近战/安全区/黄圈追加修复见上方当前记录。APK继续暂缓，版本未改。
