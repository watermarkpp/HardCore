...
func _run() -> void:
	...
	assert(sprite.position == Vector2(-112, -166) and visual.position.y == 6.0, "尸王绘制原点迁移与地面原点不一致")
	...
	assert(boss.ground_indicator_center().is_equal_approx(visual.position), "尸王锁定光圈未使用统一地面原点")
