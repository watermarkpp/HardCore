...
func _run() -> void:
	...
	assert(visual.position.y == 6.0 and is_equal_approx(boss.name_label.position.y, boss.name_label_anchor_y()), "骷髅精灵脚底/名称高度错误")
	assert(boss.name_label.position.y + boss.name_label.size.y < boss.health_bar_anchor_y(), "骷髅精灵名称压住大型造型")
