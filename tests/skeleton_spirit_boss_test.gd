...
func _run() -> void:
	...
	assert(visual.uses_final_art() and visual.frame_size == Vector2i(160, 160), "骷髅精灵Boss客户端资源未启用")
	assert(visual.actor_ground_offset == Vector2i(32, 28), "骷髅精灵未采用经典客户端角色原点迁移量")
	assert(sprite.texture.get_size() == Vector2(640, 1280) and sprite.position == Vector2(-112, -166), "骷髅精灵图集或绘制原点迁移错误")
	assert(visual.position.y == 6.0 and boss.name_label.position.y == -116.0, "骷髅精灵脚底/名称高度错误")
	...
	assert(visual.current_state == "attack" and sprite.texture.get_size() == Vector2(960, 1280), "骷髅精灵攻击动画未触发")
