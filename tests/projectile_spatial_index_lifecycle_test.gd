...
func _make_enemy(
	...
	assert(
		...
		"lifecycle fixture monster_id=%d must exist" % FIXTURE_MONSTER_ID
	)
	enemy.setup(canonical_data, null, false)
	assert(
		enemy.monster_id == FIXTURE_MONSTER_ID and not enemy.is_boss,
		"lifecycle fixture must remain an ordinary exact-ID target"
	)
	enemy.max_hp = 100
	...
	assert(
		...
		"lifecycle fixture target must survive exact-ID admission"
	)
	_index.register(
		serial,
		map_id,
