...
func _make_enemy(
	...
	assert(
		...
		"stable order fixture monster_id=%d must exist" % FIXTURE_MONSTER_ID
	)
	enemy.setup(canonical_data, null, false)
	assert(
		enemy.monster_id == FIXTURE_MONSTER_ID and not enemy.is_boss,
		"stable order fixture must remain an ordinary exact-ID target"
	)
	enemy.max_hp = 1000
	...
	assert(
		...
		"stable order fixture target must survive exact-ID admission"
	)
	_index.register(
		serial,
		1,
