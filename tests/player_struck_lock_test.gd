...
func _ready() -> void:
	...
	assert(is_equal_approx(ProfessionRules.player_struck_reaction_seconds(), 0.240), "HardCore三帧受击表现应为240ms")
	var threshold := ProfessionRules.player_struck_damage_threshold(player.max_hp)
	assert(threshold == 3, "120 最大生命的统一硬直阈值应为3点最终伤害")
	assert(str(ProfessionRules.COMBAT_REACTION_POLICY.policy_id) == "hardcore_player_hit_reaction_v2", "硬直公式必须明确标识为HardCore策略")
	...
	assert(player._struck_lock_remaining <= 0.0 and player._struck_reaction_lock_remaining > 0.0, "100ms动作锁与240ms受击表现必须分开计时")
	...
	assert(player.global_position.is_equal_approx(struck_position), "240ms边界帧仍应保持原位")
	player._physics_process(0.01)
	assert(player.velocity.x > 0.0, "240ms受击表现结束后，持续输入应恢复移动速度")
	...
	print("PLAYER_STRUCK_LOCK_PASS: HardCore threshold=%d, server lock=100ms, visual recovery=240ms" % threshold)
	get_tree().quit()
