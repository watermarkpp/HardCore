...
func _ready() -> void:
	# A non-runtime-ready monster (鹿, ID16) is authoring-allowed but must not
	# be silently promoted into a runtime snapshot.
	...
	assert(
		_has_error_prefix(blocked_validation.get("errors", []), "monster_not_runtime_ready:16:"),
