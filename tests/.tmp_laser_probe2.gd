...
func _ready() -> void:
	...
	add_child(owner)
	print("sample\tendpoint\tfitted\tdelta\tdir\tfit_axis_x\tfit_axis_y\tfit_len\tdesired_axis\tcross")
	for sample_index: int in range(16):
		...
		if close_effect == null:
			print("sample=", sample_index, " no close_effect")
			continue
		add_child(close_effect)
		if close_effect._sprites.is_empty():
			print("sample=", sample_index, " sprites empty")
			close_effect.free()
			continue
		var close_sprite := close_effect._sprites[0] as CasterSkillAnimationPlayer
		if close_sprite == null:
			print("sample=", sample_index, " sprite null")
			close_effect.free()
			continue
		var forward := close_sprite.fitted_visual_forward_extent(aim_axis)
		var diff := forward - endpoint.length()
		var raw := close_effect._visual_axis_screen_px
		print("%02d\t%8.4f\t%8.4f\t%+8.5f\t%2d\t%7.3f\t%7.3f\t%7.3f\t%8.4f\t%7.3f" % [
			sample_index,
			endpoint.length(),
			forward,
			diff,
			...
			raw.y,
			raw.length(),
			close_effect._desired_sprite_axis_extent_px,
