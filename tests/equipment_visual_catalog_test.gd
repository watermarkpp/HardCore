...
func _run() -> void:
	...
	for item_id: String in entries:
		...
		if entry.get("category", "") in VISUAL_CATEGORIES:
			...
			var expected_paper_status := "exact_client_record"
			if int(item_id) == 218:
				expected_paper_status = "user_authorized_redesign"
			elif int(item_id) == 236:
				expected_paper_status = "user_approved_project_redesign"
