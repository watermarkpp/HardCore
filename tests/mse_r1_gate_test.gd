...
func _ready() -> void:
	...
	for asset: Dictionary in assets:
		if asset.get("asset_type", "") == "ground_brush":
			ground_count += 1
			assert(Vector2i(int(asset.footprint_tiles[0]), int(asset.footprint_tiles[1])) == Vector2i.ONE)
			assert(Vector2i(int(asset.tile_size[0]), int(asset.tile_size[1])) == Vector2i(64, 32))
			assert(Vector2i(int(asset.canvas_size[0]), int(asset.canvas_size[1])) == Vector2i(64, 32))
			assert(float(asset.diamond_inner_coverage) >= 0.995)
			assert(asset.content_layer == "personal_expansion")
	assert(ground_count == 253)
