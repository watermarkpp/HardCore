...
func _ready() -> void:
	...
	for row: Variant in GameData.skills:
		...
		for field in [
			"cast_type", "target_mode", "maximum_range_gu", "search_range_gu",
			"multiplier", "area_radius_gu", "visual_radius_px", "windup",
			"hit_frame", "cooldown", "verification",
		]:
			assert(profile.has(field), "%s缺少字段%s" % [skill_name, field])
		for forbidden_field in [
			"range", "search_range", "area_radius", "area_radius_cells", "cell_size",
		]:
			assert(not profile.has(forbidden_field), "%s仍暴露无单位字段%s" % [skill_name, forbidden_field])
	...
	assert(str(fire.ui_interaction_mode) == "toggle" and str(fire.runtime_activation_mode) == "single_attack_input_direct_melee", "烈火攻击键直释开关契约错误")
	assert(str(fire.runtime_override_id) == "gameplay.warrior.fire_sword.attack_toggle.user_override.v1", "烈火用户运行时覆盖ID缺失")
	assert(str(fire.charge_state_id) == "warrior.fire_sword.charge_armed" and float(fire.charge_lifetime) == 10.0, "烈火一次性充能状态契约错误")
	...
	assert(str(lightning.target_mode) == "single" and float(lightning.search_range_gu) >= float(lightning.maximum_range_gu), "雷电术自动选敌范围错误")
	var summon := ProfessionRules.skill_combat_profile("召唤神兽")
	assert(str(summon.target_mode) == "self" and float(summon.search_range_gu) == 0.0, "召唤技能不应自动抢目标")
