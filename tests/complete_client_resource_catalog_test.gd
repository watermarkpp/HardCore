...
func _ready() -> void:
	...
	assert((helmet.get("generation", {}).get("aiPixelsLimitedTo", []) as Array).is_empty(), "运行图集不得直接复制AI概念图像素")
	assert(str(helmet.get("generation", {}).get("runtimePixelGenerator", "")).begins_with("Godot 4.7"), "运行头盔没有使用Godot正交3D渲染")
	...
	assert(str(direction_references.get("approvedConcept", "")).ends_with("black_iron_helmet_approved_concept_20260717.png"), "用户确认的黑铁头新造型没有写入来源记录")
	assert(str(direction_references.get("godotRenderer", "")).ends_with("render_black_iron_helmet_3d.gd"), "Godot头盔渲染器没有写入来源记录")
	assert(is_equal_approx(float(helmet.get("generation", {}).get("runtimeEnvelopeScale", 0.0)), 1.0), "黑铁头没有校准到客户端头盔p75尺寸")
	assert(int(helmet.get("deathPoseBaseline", {}).get("records", 0)) == 32, "死亡头盔没有8方向×4帧的一一对应记录")
