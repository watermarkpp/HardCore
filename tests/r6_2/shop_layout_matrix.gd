...
func inspect_row(key: String, domain: String, name_text: String, expected_body: String, quote: Dictionary) -> void:
	...
	check(region.grow(1.0).encloses(actual), prefix + "outside unchanged region")
	check(v.size.y + 1.0 >= v.size.x * 1.12, prefix + "portrait ratio")
...
func _run() -> void:
	if not GameData.ensure_loaded():
		push_error("R2_SHOP_MATRIX_DATA_MISSING")
		get_tree().quit(1)
		return
	...
	for i in range(offset, stop):
		...
		panel.open_for("R2目录布局测试（真实报价）", [entry], merchant)
		await frames(6)
		...
		panel._ui_dismiss_selection()
	var f := FileAccess.open("user://r2_shop_matrix_%d_%d.json" % [offset, stop], FileAccess.WRITE)
	if f != null:
		f.store_string(JSON.stringify({"schema": "hc.r2.shop_matrix.v1", "offset": offset, "end": stop,
			"catalog_keys": catalog_keys, "rows": rows, "failures": failures}, "  "))
		f.close()
	else:
		failures.append("evidence write failed")
	for e: String in failures:
		push_error("R2_SHOP_MATRIX " + e)
	print("R2_SHOP_MATRIX_%s records=%d failures=%d" % ["PASS" if failures.is_empty() else "FAIL", rows.size(), failures.size()])
