...
func _verify_laser_target_distance_direction_and_replay_stability() -> void:
	...
	for sample_index: int in range(16):
		...
		var close_forward := close_sprite.fitted_visual_forward_extent(aim_axis)
		print("sample=%02d endpoint=%0.5f fitted=%0.5f diff=%+0.5f dir=%d fit=%s" % [
			sample_index,
			endpoint.length(),
			close_forward,
			close_forward - endpoint.length(),
			close_sprite.direction_index,
			close_effect._visual_axis_screen_px
		])
		assert(is_equal_approx(
			close_forward,
			endpoint.length()
		...
		assert(far_sprite.transform.is_equal_approx(close_sprite.transform))
		assert(_assert_rect_equal(far_sprite.fitted_visual_bounds(), close_sprite.fitted_visual_bounds()))
		...
		for frame_index: int in range(far_sprite.frame_count()):
			...
			assert(far_sprite.transform.is_equal_approx(expected_longitudinal))
...
func _texture_visible_forward_interval(
	...
	for y: int in range(image.get_height()):
		for x: int in range(image.get_width()):
			...
			var projection := sprite.transform.basis_xform(local_center).dot(normalized_axis)
			minimum = minf(minimum, projection)
			maximum = maxf(maximum, projection)
	assert(is_finite(minimum) and is_finite(maximum))
...
func _verify_named_direction_forward_fact(
	...
	# These values intentionally differ by projected direction: 8 GS is about
...
func _assert_rect_equal(left: Rect2, right: Rect2) -> bool:
	var position_equal := left.position.is_equal_approx(right.position)
	var size_equal := left.size.is_equal_approx(right.size)
	return position_equal and size_equal
