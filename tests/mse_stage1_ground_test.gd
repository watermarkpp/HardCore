...
func _ready() -> void:
	...
	assert(manifest.coordinate_contract_id == MapEditorCoordinate.GROUND_COORDINATE_CONTRACT_ID)
	assert(state.coordinate_contract_id == MapEditorCoordinate.GROUND_COORDINATE_CONTRACT_ID)
	assert(document.ground.coordinate_contract_id == MapEditorCoordinate.GROUND_COORDINATE_CONTRACT_ID)
	assert(document.ground.tile_anchor_mode == "cell_center")
	assert(document.ground.workspace_manifest == document.editor_meta.workspace.path_join("ground/ground_manifest.json"))
	assert(document.ground.workspace_state == document.editor_meta.workspace.path_join("ground/ground_state.json"))
	...
	assert(bich.ground.origin_px == [2560.0, 16.0])
