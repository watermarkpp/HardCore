...
func _ready() -> void:
	...
	player.attack_min = 1
	player.attack_max = 1
	assert(is_equal_approx(player.attack_cooldown, 0.85), "战士普通攻击间隔必须为850ms")
	...
	assert(not player.can_start_attack(), "动作结束后仍应等待850ms攻击间隔")
	player._physics_process(0.34)
	assert(player.can_start_attack(), "850ms攻击间隔结束后应允许下一刀")
	print("WARRIOR_ATTACK_TIMING_PASS：850ms续刀间隔、510ms完整动作锁和独立计时正常")
	get_tree().quit()
