...
The pre-R2/R3 approved set (120 ids) must never regress, every runtime_allowed
monster must satisfy the full closure contract, and every version_difference
monster must remain runtime-disabled.
"""
...
BASELINE_PATH = ROOT / "outputs/monster_runtime_r2r3_baseline.json"


def main() -> None:
    baseline = json.loads(BASELINE_PATH.read_text(encoding="utf-8"))
    baseline_ids = set(int(x) for x in baseline["ids"])
