...
func _ready() -> void:
	...
	# Q3-C: legacy the legacy resolver was removed; the merged visual
	# profile is read directly from the canonical visual registry.
	var visual_profile: Dictionary = merged_profile
	...
	assert(visual_profile is Dictionary, "%s visual profile should exist" % skill_id)
	...
	assert(visual_profile.get("animation", {}).get("contract", "") == "caster_skill_animation.v1", "%s registry profile should keep runtime animation contract" % skill_id)
