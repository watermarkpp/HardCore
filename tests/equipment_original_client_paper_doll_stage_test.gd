...
func _run() -> void:
	...
	_assert_int_array(manifest.get("viewportOrigin", []), [0, -44], "viewportOrigin")
	_assert_int_array(manifest.get("viewportBounds", []), [0, -44, 232, 281], "viewportBounds")

	var composition: Dictionary = manifest.get("composition", {})
	_assert_int_array(composition.get("baseScreenOrigin", []), [0, 0], "baseScreenOrigin")
	_assert_int_array(composition.get("equipmentScreenAnchor", []), [31, 96], "equipmentScreenAnchor")
	_assert_int_array(composition.get("viewportOrigin", []), [0, -44], "composition.viewportOrigin")
	...
	_assert_int_array(stage.get("stagePosition", []), [7, -44], "stagePosition")
