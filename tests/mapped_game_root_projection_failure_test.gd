...
func _run() -> void:
	...
	assert(
		MapEditorRuntimeBridge.registry_generation()
			== registry_generation_before + 1,
		"registry invalidation must advance projection cache generation"
	)
	var formal_after_invalidate: Dictionary = game._resolve_projection_profile_for_map(
		MapEditorRuntimeBridge.BICH_MAP_ID
	)
	assert(not formal_after_invalidate.has("_cache_probe"), "registry generation did not invalidate projection cache")
