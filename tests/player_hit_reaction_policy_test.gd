...
func _run() -> void:
	...
	assert(str(player.visual._action_name) == "attack", "普通受击不得取消已提交的攻击动画")
	...
	assert(str(player.visual._action_name) == "hit", "攻击完成后必须播放排队的受击表现")
	...
	assert(str(player.visual._action_name) == "attack", "多次受击不得打断当前攻击动作")
	player._attack_action_timer = 0.0
	player._physics_process(1.0 / 60.0)
	...
	assert(str(player.visual._action_name) == "hit", "合并后的受击表现必须播放")
