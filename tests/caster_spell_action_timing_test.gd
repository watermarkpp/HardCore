...
const SPELL_ACTION_DURATION := 0.60
...
func _ready() -> void:
	...
	print("CASTER_SPELL_ACTION_TIMING_PASS：法师/道士使用600ms施法动作锁，释放点与冷却独立")
	get_tree().quit(0)
...
func _verify_all_active_caster_cooldowns() -> void:
	for stable_id: String in ProfessionRules.SKILL_CATALOG:
		...
		if str(profile.get("profession_id", "")) not in ["wizard", "taoist"] or str(profile.get("cast_type", "")) == "passive":
			continue
		assert(float(profile.get("cooldown", 0.0)) > SPELL_ACTION_DURATION, "%s主动技能冷却不得与600ms身体动作混用" % stable_id)
...
func _verify_caster(profession: String, skill_name: String) -> void:
	...
	assert(is_equal_approx(float(profile.get("action_duration", 0.0)), SPELL_ACTION_DURATION), "%s施法身体动作不是主源6帧×100ms" % profession)
	assert(int(profile.get("action_frame_count", 0)) == 6 and int(profile.get("action_frame_time_ms", 0)) == 100, "%s施法帧契约不完整" % profession)
	...
	assert(is_equal_approx(player.visual._action_duration, SPELL_ACTION_DURATION), "%s身体动画没有使用600ms动作时长" % profession)
	...
	assert(player._attack_action_timer <= 0.0, "%s施法动作锁没有在600ms后结束" % profession)
...
func _verify_cast_speed_does_not_shorten_body_action() -> void:
	...
	assert(is_equal_approx(player._attack_action_timer, SPELL_ACTION_DURATION), "施法速度错误缩短主源600ms身体动作")
