...
func _run() -> void:
	...
	assert(WarriorCombatMath.slaying_proc_cycle(0) == 7 and WarriorCombatMath.slaying_proc_cycle(3) == 4, "攻杀触发周期错误")
	assert(WarriorCombatMath.slaying_damage(20, 0) == 25 and WarriorCombatMath.slaying_damage(20, 3) == 28, "攻杀附加伤害错误")
	...
	print("WARRIOR_SERVICE_FORMULA_PASS：六技能ID、命中、伤害、范围规则、野蛮概率与客户端510ms动作已按源码校准")
	get_tree().quit(0)
