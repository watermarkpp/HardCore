...
func _make_enemy(center_ground_gu: Vector2, serial: int) -> EnemyActor:
	...
	assert(
		...
		"query-order fixture monster_id=%d must exist" % FIXTURE_MONSTER_ID
	)
	enemy.setup(canonical_data, null, false)
	assert(
		enemy.monster_id == FIXTURE_MONSTER_ID and not enemy.is_boss,
		"query-order fixture must remain an ordinary exact-ID target"
	)
	enemy.max_hp = 1000
	...
	assert(
		...
		"query-order fixture target must survive exact-ID admission"
	)
	_index.register(
		serial,
		1,
