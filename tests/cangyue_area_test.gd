...
func _run() -> void:
	...
	assert(archer.attack_range_gu >= 200.0 / 32.0, "骷髅弓箭手远程行为失效")
	archer.free()
	var mage := EnemyActor.new()
	mage.setup(GameData.get_monster("牛魔法师"), game.player, false)
	assert(mage.attack_range_gu >= 200.0 / 32.0, "牛魔法师远程行为失效")
	mage.free()
	var priest := EnemyActor.new()
	priest.setup(GameData.get_monster("牛魔祭司"), game.player, false)
	assert(priest.attack_range_gu >= 170.0 / 32.0 and priest.life_steal_ratio > 0.0, "牛魔祭司恢复行为失效")
	priest.free()
