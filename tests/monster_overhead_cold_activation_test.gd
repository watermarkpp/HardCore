...
func _run() -> void:
	...
	for monster_id: int in SAMPLE_MONSTER_IDS:
		...
		_assert_all_visible_frames_below_overhead(enemy, monster_id)
	...
	MonsterVisual.reset_client_resource_cache()
	print("MONSTER_OVERHEAD_COLD_ACTIVATION_PASS small/medium/large/boss async profiles refresh above every visible frame")
	get_tree().quit(0)


func _assert_all_visible_frames_below_overhead(enemy: EnemyActor, monster_id: int) -> void:
	...
	for action_name: String in ACTIONS:
		...
		for direction in range(8):
			for frame_index in range(frame_count):
				...
				var visible_top_canvas_y: float = (sprite.get_global_transform_with_canvas() * Vector2(0.0, float(alpha_top))).y
				assert(bar_bottom_canvas_y <= visible_top_canvas_y - REQUIRED_BODY_MARGIN, "monsterId=%d %s direction=%d frame=%d barBottom=%s visibleTop=%s" % [monster_id, action_name, direction, frame_index, bar_bottom_canvas_y, visible_top_canvas_y])
