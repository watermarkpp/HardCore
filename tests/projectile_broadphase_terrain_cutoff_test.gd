...
func _make_enemy(
	...
	assert(
		...
		"terrain cutoff fixture monster_id=%d must exist" % FIXTURE_MONSTER_ID
	)
	enemy.setup(canonical_data, null, false)
	assert(
		enemy.monster_id == FIXTURE_MONSTER_ID and not enemy.is_boss,
		"terrain cutoff fixture must remain an ordinary exact-ID target"
	)
	enemy.max_hp = 1000
	...
	assert(
		...
		"terrain cutoff fixture target must survive exact-ID admission"
	)
	_index.register(
		serial,
		1,
