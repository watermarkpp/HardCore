...
func _ready() -> void:
	...
	assert(bich.design.design_size == [256, 256])
	...
	var palette := MapAssetCatalogService.palette_assets("bich")
	assert(palette.size() >= 415 and palette[0].asset_id == "builtin.blank_old_grass")
	...
	assert(editor.asset_tree.get_root() != null)
	assert(editor._first_asset_tree_item() != null)
	assert("sandbox_64.editor.json" in editor.path_label.text)
	editor._create_map("bich_province", "outdoor_province", 4, "比奇省")
	assert(editor.current_document.design.design_size == [256, 256])
	editor.queue_free()
