...
func _ready() -> void:
	...
	assert(MapEditorGameplaySemanticService.add_entry(document, "safe_area", Vector2i(32, 32), {"radius_gu":8.0}).ok)
	assert(MapEditorGameplaySemanticService.add_entry(document, "monster_spawn", Vector2i(66, 48), {"content_id":"monster.strawman", "monster_id":"monster.strawman", "radius_gu":5.0}).ok)
