...
func _run() -> void:
	...
	assert(panel.confirmation_popup.get_meta("stable_id", "") == "ui.confirmation.dialog", "职业面板没有复用公共确认组件")
	assert(panel.confirmation_popup.current_request.context.profession_id == "wizard", "确认请求没有保留职业稳定 ID")
	panel.confirmation_popup.cancel_button.pressed.emit()
	assert(PlayerState.profession == "战士" and not panel.confirmation_popup.visible, "取消职业切换后修改了角色数据或没有关闭弹窗")
	panel._request_confirmation()
	panel.confirmation_popup.confirm_button.pressed.emit()
	await get_tree().process_frame
