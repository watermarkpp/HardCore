...
func _make_enemy(game: Node, caster: PlayerCharacter, position: Vector2) -> EnemyActor:
	...
	assert(
		...
		"mapped snapshot fixture monster_id=%d must exist" % FIXTURE_MONSTER_ID
	)
	enemy.setup(canonical_data, caster, false)
	assert(
		enemy.monster_id == FIXTURE_MONSTER_ID and not enemy.is_boss,
		"mapped snapshot fixture must remain an ordinary exact-ID target"
	)
	enemy.max_hp = 9999
	...
	assert(
		...
		"mapped snapshot fixture target must survive exact-ID admission"
	)
	return enemy
