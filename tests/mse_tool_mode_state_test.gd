...
func _ready() -> void:
	...
	# 2026-09-20 user order: the legacy grid-collision menu (manual collision
	# draw / single-cell erase / whole erase / walkability preview toggle) was
	# removed; the polygon tool (自由多边形 R2 panel) is the only collision
	# editor. The legacy-mode drives that lived here are gone; the polygon
	# tool has its own behavior tests (poly_*) and the shortcut test covers
	# its T toggle.
