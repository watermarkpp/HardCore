...
func _ready()->void:
	...
	assert((portals_by_target[217].screen_position_px as Vector2).is_equal_approx(MapEditorRuntimeBridge.grid_cell_to_screen_position_px(runtime, [72, 5])))
	assert((portals_by_target[268].screen_position_px as Vector2).is_equal_approx(MapEditorRuntimeBridge.grid_cell_to_screen_position_px(runtime, [6, 5])))
