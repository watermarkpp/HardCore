...
func _assert_bridge_build_metrics(stats: Dictionary) -> void:
	...
	assert(int(stats.wall_resolve_queries) > 0)
	assert(int(stats.wall_owner_samples) > 0)
	assert(int(stats.wall_base_samples) >= 0)
	assert(int(stats.wall_relation_skips) >= 0)
