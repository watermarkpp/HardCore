...
func _run() -> void:
	...
	assert(special.size() == 11, "special count %d" % special.size())
	assert(unresolved.size() == 160, "unresolved count %d" % unresolved.size())
	...
	var dark_rainbow := MapEditorContentCatalogService.find_by_monster_id("unresolved_monster", 240)
	assert(not dark_rainbow.is_empty(), "ID 240 暗之虹魔教主 missing from unresolved_monster")
	assert(MapEditorContentCatalogService.find_by_monster_id("special_monster", 240).is_empty(), "ID 240 must NOT be in special_monster")
