...
def main() -> None:
    ...
    assert target["itemId"] == 151
    assert target["visualAssetId"] == "black_iron_golden_151"
    assert target["displayName"] == "黑铁头盔"
    ...
    assert target["provenance"]["derivation"] == (
        "alpha_bounds_crop_only_no_matte_removal_no_resample"
    )
