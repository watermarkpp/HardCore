...
func _run() -> void:
	...
	assert(str(semantic.get("schema", "")) == "hardcore.dpv2.monster_drop_semantic_authority.v1")
	assert(str(semantic.get("authority_id", "")) == "dpv2.monster_drop_semantic.v1")
	assert(str(semantic.get("status", "")) == "SEMANTIC_AUTHORITY_COMPLETE")
	assert(bool(semantic.get("production_active", false)))
	var semantic_summary: Dictionary = semantic.get("summary", {})
	assert(int(semantic_summary.get("canonical_monsters", -1)) == 156)
	assert(int(semantic_summary.get("runtime_allowed", -1)) == 153)
	assert(int(semantic_summary.get("drop_enabled", -1)) == 144)
	assert(int(semantic_summary.get("explicit_non_loot", -1)) == 9)
	assert(int(semantic_summary.get("runtime_disabled", -1)) == 3)
	assert(int(semantic_summary.get("production_slots", -1)) == 6809)
	...
	print("DPV2_21CQ_DIRECT_LOADER_PASS: manifest=1 hashes=1 semantic=156/153/144/9/3 slots=6809 exact_id_api=1")
	get_tree().quit(0)
