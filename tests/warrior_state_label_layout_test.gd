...
func _run() -> void:
	...
	var peak_source: Vector2 = ChassisDesigns.active_design()["center_peak_source"]
	var peak_y: float = (chassis.get_global_transform() * hud._chassis_source_to_local(peak_source)).y
