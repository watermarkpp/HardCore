...
- 只有主源对当前精确目标被证明为 `missing`、`unusable` 或 `incompatible` 时，才允许按 `auxiliary_1`、`auxiliary_2`、`auxiliary_3` 的固定顺序逐级检索。进入任一级前，必须记录所有更高优先级来源的检索对象、实际路径、版本或哈希、查询结果和逐项拒绝原因；没有这份证据即视为未检索主源，禁止采用低级来源。
