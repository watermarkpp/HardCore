...
- 用户已对本项目范围内除删除现有文件或数据之外的全部操作给予持续、最高级别的直接执行授权，包括读取、修改、新建、测试、提交、合并、构建、导出、部署和设备调试。主任务与专业工作树不得就这些操作、实现选择或测试步骤向用户请求确认，必须依据现有证据自行裁决并持续执行到完成；只有在即将删除现有文件或数据前才需要向用户确认。任务若需要用户亲自完成工具无法代办的外部实体动作（例如重新插入已拔出的手机），只发送简短操作通知，不将其作为授权询问，也不得把项目内部决策转交给用户。
