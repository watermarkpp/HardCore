...
func _load_active_patch() -> void:
	if not FileAccess.file_exists(ACTIVE_MANIFEST):
		return
	var parsed: Variant = JSON.parse_string(FileAccess.get_file_as_string(ACTIVE_MANIFEST))
	...
	if expected_size <= 0 or expected_size > MAX_PATCH_BYTES:
		...
		return
	var pack_path := PATCH_DIR + "/" + file_name
	if not FileAccess.file_exists(pack_path):
		load_error = "pack_missing"
		return
	var bytes := FileAccess.get_file_as_bytes(pack_path)
	if bytes.size() != expected_size:
		load_error = "pack_size_mismatch"
		return
	if FileAccess.get_sha256(pack_path).to_upper() != expected_hash:
		load_error = "pack_hash_mismatch"
		return
