...
static func _validated_animation_catalog() -> Dictionary:
	# The current main-tree catalog is the sole authoring roster. Validate its
	# self-described summary and row identities without pinning an old row count
	# or hash, so the latest canonical catalog remains usable as it evolves.
	if not FileAccess.file_exists(ANIMATION_CATALOG_PATH):
		return {}
	...
	if not summary is Dictionary or not rows is Array or rows.is_empty():
		return {}
	var total := int(summary.get("total", -1))
	var formal := int(summary.get("formal", -1))
	var provisional := int(summary.get("provisional", -1))
	var missing := int(summary.get("missing", -1))
	if (
		total != rows.size()
		or formal != rows.size()
		or provisional != 0
...
static func formal_entry(monster_id: int) -> Dictionary:
	# One read-only view of all formal authorities: contact geometry is the
	# base, calibration is the reviewed runtime projection, and the manual
	# contract contributes the saved pose/offset.
...
static func _source_evidence_matches(
	...
	for key in [
		...
		var expected_hash := str(
			expected.get("%sSha256" % key.trim_suffix("Path"), "")
		)
