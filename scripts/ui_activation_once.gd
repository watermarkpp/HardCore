...
func _end(kind: String, pointer: int, point: Vector2, cancelled: bool = false) -> void:
	...
	# Input's touch-to-mouse emulation can deliver mouse UP before touch UP.
	# BaseButton then emits pressed before our authoritative touch release and
	# emits nothing on the later touch UP. Commit the validated native release
	# itself; the same serial reservation still rejects any native duplicate.
	if kind == "touch" and _release_serial >= 0:
		_commit()
