...
func _ready() -> void:
	...
	item_label.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT, Control.PRESET_MODE_MINSIZE, 5)
...
func setup(data: Dictionary) -> void:
	...
	var estimated_width := clampi((item_name.length() + suffix.length()) * 17 + 28, 96, 236)
	size = Vector2(estimated_width, 32)
	if is_node_ready():
		_apply_data()
