...
static func _defaults(kind: String, tile: Vector2i, properties: Dictionary) -> Dictionary:
	...
	match kind:
		...
		"monster_spawn": entry.merge({"monster_id": -1, "count": 1, "max_alive": 1, "respawn_seconds": 60, "radius_tiles": 2, "spawn_rule": "ambient"})
		"boss_spawn": entry.merge({"monster_id": -1, "count": 1, "max_alive": 1, "respawn_seconds": 1800, "radius_tiles": 0, "spawn_rule": "boss"})
...
static func _validate(document: Dictionary, kind: String, tile: Vector2i, properties: Dictionary) -> Array[String]:
	...
	if kind in ["monster_spawn", "boss_spawn"] and int(properties.get("monster_id", -1)) <= 0:
		errors.append("positive_monster_id_required")
