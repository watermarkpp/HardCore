	...
	for layer:String in KIND_TO_LAYER.values():
		...
		for index in entries.size():
			...
			if str(entries[index].get("kind", "")) == "safe_area" and str(entries[index].get("shape", "circle")) == "polygon":
				...
				for raw_point: Variant in entries[index].get("polygon_ground_gu", []):
				...
				entries[index]["polygon_ground_gu"] = shifted_points
...
static func duplicate_entry_snapshot(
	...
	if kind == "safe_area" and str(source_entry.get("shape", "circle")) == "polygon":
		...
		for raw_point: Variant in source_entry.get("polygon_ground_gu", []):
			if not raw_point is Array or raw_point.size() != 2:
		...
		properties["polygon_ground_gu"] = translated_polygon
...
static func _defaults(kind: String, tile: Vector2i, properties: Dictionary) -> Dictionary:
	...
	match kind:
		...
		"safe_area": entry.merge({"area_id": "safe.unassigned", "shape": "circle", "radius_gu": 4.0, "polygon_ground_gu": [], "blocks_pvp": true, "blocks_monster_damage": true, "return_anchor": false})
		"light": entry.merge({"light_id": "light.unassigned", "radius_gu": 3.0, "color": "#f2b96d", "intensity": 1.0, "flicker": false})
		"region_trigger": entry.merge({"trigger_id": "trigger.unassigned", "radius_gu": 2.0, "trigger_type": "enter", "action": "dialogue_or_event", "once": false})
...
static func _validate(document: Dictionary, kind: String, tile: Vector2i, properties: Dictionary) -> Array[String]:
	...
	if kind == "safe_area" and str(properties.get("shape", "circle")) == "polygon":
		var polygon: Array = properties.get("polygon_ground_gu", [])
		if polygon.size() < 3:
	...
	if kind in ["safe_area", "light", "region_trigger"] and str(properties.get("shape", "circle")) != "polygon" and float(properties.get("radius_gu", 0.0)) <= 0.0:
