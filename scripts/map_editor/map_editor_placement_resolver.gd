...
static func resolve(document: Dictionary, asset_id: String, tile: Vector2i, layer := "object_base", role := "decoration") -> Dictionary:
	...
	# Wall packages define their anchor at the centre of the first occupied
	# tile. Long walls must not be shifted to the centre of the full footprint.
	var foot_tile := Vector2(tile) + Vector2(0.5, 0.5)
	if str(asset.get("asset_type", "")) != "wall_module":
		foot_tile = Vector2(float(tile.x)+float(footprint[0])*.5,float(tile.y)+float(footprint[1])*.5)
