class_name MapEditorWallRenderPublishService
extends RefCounted
## Shared deterministic derived-wall publisher for the editor and build tool.
## Only derived plans and content-addressed PNGs are written; map authoring,
## runtime geometry, collisions and the release registry remain unchanged.
...
# Alternate output directories permit isolated publication validation without
# modifying the formal artifact store. Editor/CLI retain the formal defaults.
var plan_directory := PLAN_DIR
var store_directory := STORE_DIR
var staging_directory := "res://outputs/wall_perf/staging"
var _image_cache := {}
...
func publish_map(map_key: String) -> Dictionary:
	if map_key.is_empty() or map_key.get_file() != map_key or map_key.contains(".."):
		return {"error": "invalid map key"}
	_image_cache.clear()
	var runtime_path := (
	...
	# Published maps pin their visual asset metadata. Match the runtime's
	# snapshot lookup so later catalog edits cannot rebake a different scene.
	var commands: Array = GEOMETRY_SERVICE.sorted_draw_commands(
		instances, raw.get("visual_asset_snapshot", {})
	)
	...
	# plan untouched.
	var candidate_path := ProjectSettings.globalize_path(plan_directory).path_join(
...
func _store_png(image: Image, store_names: Dictionary) -> Dictionary:
	var staging := ProjectSettings.globalize_path(staging_directory)
	...
	var store_global := ProjectSettings.globalize_path(store_directory)
	...
	return {
		"path": "%s/%s.png" % [store_directory.replace("res://", ""), sha],
		"sha256": sha,
...
func _stage_plan_document(map_key: String, plan_document: Dictionary) -> String:
	var plan_dir := ProjectSettings.globalize_path(plan_directory)
...
func _load_image(path: String) -> Image:
	...
	# Preserve the accepted import processing (including alpha-edge colors),
	# but bypass long-lived editor cache entries after an asset reimport.
	if ResourceLoader.exists(resource_path):
		var texture := ResourceLoader.load(resource_path, "Texture2D", ResourceLoader.CACHE_MODE_IGNORE) as Texture2D
		if texture != null:
