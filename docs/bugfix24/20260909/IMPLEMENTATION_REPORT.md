...
| W4 音频 | 代码及早期六条件观测已集成；八条件全活跃真实帧对照通过，证据归档中 | 每条件20/50 actor均真实攻击；不宣称设备混音或普遍加速 |
| W5 属性面板 | 用户已通过；字号20/14已集成dd919c21 | 主树inventory/祝福显示2/2 PASS；银行按钮另行接线 |
| W3 Loading/升级、W7 事务 | 核心、银行、完整掉落实例已提交并通过专项 | 全部延迟敌人攻击 epoch 的V3接线仍待最终集成 |

## 分段证据（按施工过程追加，不是全包验收）

本节早期“正在实现/不予接受”是当时状态，后续提交与证据取代其当前状态；最新逐项结论见 ACCEPTANCE_MATRIX.md。
...
W4后续严格对照：旧版cf1d2718与候选ef827331在相同正式地图夹具及同字节headless兼容overlay下，各完成20/50 × on/off四条件，总八条件；每条件240 physics ticks，每个actor至少一次真实攻击、sequence与请求逐项对账。runner分别154446_265_1896与154640_120_10924，均自然退出PASS。开启音效的整帧墙钟p95：50只旧12.794ms/新12.348ms，20只旧9.374ms/新9.305ms。保留on/off攻击计时器相位导致请求数不同的限制，不当成等请求量收益；服务调用wall不是音频线程CPU/混音。原始stderr仍有ObjectDB与退出资源报告，未改变allowlist；Android与设备听感未验。
