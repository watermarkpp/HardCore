...
This scene verifies exact monster_id routing for existing EnemyActor delivery paths. It does not modify production scripts, formal data, global schema, allowlists, caches, or the 12 DATA_HOLD IDs. It is authored in the UI specialist tree against base cf1d2718befdef7e6cc2fb274fd91ce0759d105f. **Status: NOT_RUN.** The production exact-ID mapping/gate is not yet present; integration must rerun the scene on the actual integrated baseline after Sol's W1 production mapping lands. This turn only writes static fixtures; Godot/import was not started and no PASS is claimed.
...
The result must contain `W1_EXACT_RANGED_DELIVERY_PASS` plus nine `W1_SPECIAL_NONCOMBAT_HOLD` lines, with runner JSON/raw logs saved at the same final code/dependency HEAD. Current status is static-only and waiting for Sol's exact-ID `combatEnabled=false` gate; until that production gate lands, the negative actor matrix is expected to fail rather than accept autonomous melee. Do not downgrade to profile injection, inherit candidate class behavior, or treat blank delivery as melee. The other six DATA_HOLD IDs (`41,59,78,123,161,190`) remain absent; 228–233 are covered only by the fixed-noncombat negative case and still have no actor class or delivery mapping.
