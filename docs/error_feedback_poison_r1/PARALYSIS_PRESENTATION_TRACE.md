...
> **R1.1 修订记录（20260918，控制器复审后）**：本文件的运行时追踪（状态源/入口/退出/各跳证据）经复核仍然有效，按 R1 原样保留。但 R1 据此得出的表现结论已被推翻：追踪当时只找到了脚下蓝环这一条表现路径，就推断"血条下标志在 BASE 不存在、蓝环应保留"，属于"发现当前代码与用户明确描述不一致后自行改写用户要求"。R1.1 复核确认：玩家侧本无血条下标志，正确做法是按用户描述新建（怪物侧已有 `enemy.gd` 血条下点阵先例可循）。现玩家麻痹/中毒共用 `PlayerStatusMarkerStrip`（血条下固定槽位），脚下蓝环已随 R1.1 移除，怪物脚下控制环未动。见 `docs/error_feedback_poison_r11/IMPLEMENTATION_REPORT.md`。
