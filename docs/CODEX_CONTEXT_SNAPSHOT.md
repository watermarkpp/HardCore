...
更新时间：2026-07-28 22:46（Asia/Shanghai）
...
- 分支/运行时代码基线：`codex/integration` @ `f6cec727`（本次快照提交只改本文档）。
...
| `HardCore-worktrees/equipment` | `codex/equipment` @ `5be70aa5` | 4 个既有 monster import 修改及 UID/生成项继续保护 | 149 八方向透明原切片与第九张背包专用素材已进入校准工具；146/147 用户草稿保持冻结，正式游戏素材未烘焙，继续等待其他头盔全部校准后统一接入 |
