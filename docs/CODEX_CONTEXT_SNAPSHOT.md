...
更新时间：2026-07-27 18:07（Asia/Shanghai）
...
- 分支/运行时代码基线：`codex/integration` @ `443c08c8`（其后的快照提交只改本文档）。
...
- equipment helmets：12 itemId/11 视觉身份、66 物理 atlas、2784 逻辑帧、6 动作×8 方向、透明角、Hair 逐帧锚点/SHA 溯源与 StateItem 世界像素零复用通过；法神头盔 `item_id=236` 使用用户授权原图与单目标构建器完成全遮脸替换，并按默认尺寸硬限制世界像素为最大 `19×23px`、纸娃娃内容为最大 `24×29px`；非 236 文件及合同数据零变化，Godot 头盔专项、视觉目录与真实角色八方向合成通过。
...
| `HardCore-worktrees/equipment` | `codex/equipment` @ `bae1d81` | 4 个既有 monster import 修改及 UID/生成项继续保护 | 圣战/法神默认尺寸修正分别已集成为 `ff30c3ed` / `443c08c8` |
