...
- 持久单独增加不算JP、地面图标层级与事件驱动名称排布、10项精确分类、沃玛教主命运之刃新增1/24槽、共用Buff图标行；12类神水实际使用/到期恢复属性/消失图标/续时验证PASS。原掉率108槽、优先级和15上限保留。
- 31不同场景分批最终PASS，固定源码4/4 PASS；桌面 `HardCore-1.0-v81-release.apk`，466,010,045 bytes，SHA256 `6AF911A381AA5D567B6086D698EC4480FAC9316192D4E6BAEE50B96E4705D89E`。同签名覆盖及包内身份/冻结校验PASS，228其他脚本/8983其他资源项与v80相同。
- 源码里程碑`milestone-hardcore-1.0-v81`，后续提交仅补证据文档；旧包与标签保留。ADB无设备，DEVICE NOT_RUN，冷UI、故障回滚和多怪安卓性能债务仍保留。完整证据见 `loot_ui_20260914/DELIVERY.md`。

## 2026-09-14：人物成长、神水、过滤与掉率 v80

- 源码 `fa4a1ffa88f7d2ec20d83a8548afc2d4b99d1393`：三职业完整主源成长、神水到期/无损旧堆叠、当前仓库页整理、JP范围合并及★、药水颜色与说明、三挡即时地面过滤和600秒回收。新掉率按用户DOCX倍率，Excel只比对旧台账。
- 26不同专项最终PASS，固定源码3/3 PASS。v80 APK桌面 `HardCore-1.0-v80-release.apk`，SHA256 `D8B9E5FF0C395F354997A0F0ED8E7E4F996C1433191D07C04CA5C7F76746F52D`，465,996,946 bytes，包内身份/签名/冻结资源验证PASS。
- integration快进接入，里程碑 `milestone-hardcore-1.0-v80` 固定APK源。221其他脚本/8979其他资源项与v79相同，校准/装备主表/地图未改。原版本包和标签保留；用户AGENTS.md及旧未跟踪内容保留。
- 设备NOT_RUN；性能仍须区分桌面正常事务、冷UI、故障回滚和手机实测。历史V505整表生成器的旧目录哈希锁不适用当前已正式变更的怪物目录，原台账完整审计和新倍率生成器PASS。详情见 `progression_loot_20260913/DELIVERY.md`。
...
- 输入生命周期、双指所有权、模拟鼠标与菜单/暂停/失焦/Loading移动清零完成专项；6 Pro普通聊天复审新增边界已修复。未知补丁身份拒载，已知旧版定向退役PCK，人物/仓库字节保留专项通过。
- Critical318/344的26个失败全部主树分批复验通过；34项影响回归及三轮渲染通过。最终909输入边界3/3通过；不是最终HEAD整套重跑。
- 最终严格性能12/12 FAIL，原始8次运行全部保留在 `docs/bugfix24/20260909/evidence/v3_final_8bf77a34/`，66文件SHA通过；不得用runner退出PASS替代性能门槛。后续输入改动未重测性能。
- APK72 / 1.21.0-r3-closure以同包名/同签名覆盖桌面v71为目标；最终成品身份见施工列表与交付证据。尚无设备覆盖安装、故障手机修复确认及音频听感验收。
- 后续从 `docs/bugfix24/20260909/FINAL_REVIEW.md` 与 `CONSTRUCTION_CONTENTS.md` 继续；12条DATA_HOLD与小极品5%单项+1工程默认边界保留。冻结数据核对见 `frozen_scope_final.json`。
## 2026-09-05：审计升级 APK 已完成，设备安装延期

- 构建源固定 `52ae0565856c2d99a28639b2bf0c6278186e0858`（`codex/integration` 的已提交内容）；后续提交仅记录交付证据时，不混淆包内 build-info 与文档 HEAD。
- 已接入存档业务校验、业务事务、地图删除保护、启动失败恢复、补丁有界哈希、稳定空间排序与时钟修正；修复真实 Home 同步失败门禁及 canonical 快照跨请求只读身份。详细范围/限制见 `AUDIT_UPGRADE_20260905.md`。
- 用户原有 391 项文件存在状态/哈希不变。仅经最新明确授权删除比奇重复老兵 `npc_000008`，保留 `npc_000005`；其余 NPC、82 条怪物刷新、位置、碰撞不变。
- 全量 critical 295/298 PASS，三项后续在干净构建源上 3/3 PASS。只补两项测试异步加载退出等待，不改变生产加载或近战。原始失败与后续通过均记录；不是一次 298/298 全绿。
- 桌面 APK `HardCore-20260905-audit-milestone-debug.apk`，449,193,057 bytes，SHA-256 `26584B871F61B2F6EC2DDDAF52432263E7D68A6B8BEA35B2A3196D59D5B21664`；版本 `70 / 1.19.0-audit-milestone`，包 ID 保持 `com.personal.mafaoffline`。v2 签名、运行时探针、67 图/208 chunk 闭包通过，126 个 CRLF 检出差异有逐项内容等价证明。
- 用户要求先拔手机，回来再安装。本轮不做设备写入；三档现有 40 级、空槽规范化、停用旧 active patch 与新包冷启动验收留待设备连接后。不得自动关机或声称已经安装。
- 主树和里程碑标签已同步核对。旧构建安装包/补丁/日志/未跟踪 UID 逐项归档校验后，共移除 13 个精确构建工作树，磁盘采样净增约 48.475 GiB；共享工具、源码、重要证据及最终包保留。详见 `audits/20260905/CLEANUP_COMPLETED.md`，设备安装仍延期。
