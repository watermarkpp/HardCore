...
11. 先用 64×64 沙盒地图贯通 Stage 0–8；用户于 2026-07-17 最终确认比奇为 80×80，并以 `80÷256=0.3125` 统一缩放后续地图规划尺寸。当前 64×64 比奇内容居中迁入 80×80，四周各补 8 格；客户端 700×700 `0.map` 仅保留为结构、来源与生态审计参考。
12. 地图编辑器必须从 `map_blank_templates.json` 提供 21 张地图模板下拉选择；比奇模板项优先打开现有正式工作地图，避免覆盖已完成工作，其余模板只建立空图层与虚拟空白地面，不复制其他地图内容。
