...
appearance, drop profile, and runtime semantics are all already complete and
the only remaining blocker is the legacy frozen placement_allowed=false in the
classification authority.  Flipping that single flag (and re-generating the
catalog) lets runtime_allowed become true without hand-editing runtime_allowed.

SPECIAL_RUNTIME_SEMANTICS is a formal blocker: a special monster whose runtime
semantics are not approved is never treated as policy-only.
"""
...
def _policy_only_ids(audit: dict) -> list[int]:
    ...
    for record in audit["records"]:
        blockers = set(record["runtime_blockers"])
