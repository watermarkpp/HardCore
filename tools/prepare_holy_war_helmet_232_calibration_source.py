...
SOURCE_SHA256 = (
    "486b7fb95af5f24b61d0fecfb9bbd22f9b1e3550189c5cedc1b8d264452dbec2"
)
EXTERNAL_SOURCE = Path(
    "C:/Users/Administrator/Desktop/sucai/装备/"
    "6ec29303-f433-4635-9890-8edc2f98d872.png"
)
SOURCE = (
    ROOT
...
def alpha_crop(image: Image.Image, label: str) -> Image.Image:
    ...
    if box is None:
        raise ValueError(f"empty item-232 view: {label}")
...
def import_project_source() -> None:
    ...
    if not EXTERNAL_SOURCE.exists():
        raise FileNotFoundError(
            "item-232 project source is absent and the authorized external "
...
def prepare(check: bool) -> dict[str, object]:
    ...
    if not SOURCE.exists() or file_sha256(SOURCE) != SOURCE_SHA256:
        raise AssertionError("item-232 user source hash changed")
    source = Image.open(SOURCE).convert("RGBA")
    if source.size != SOURCE_SIZE:
        raise AssertionError(
            f"item-232 user source size changed: {source.size}"
        )
