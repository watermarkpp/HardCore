...
SOURCE_SHA256 = (
    "ed3aa3aae0c106110c24ccbf43a17e71e2d7c418e2ecdbc12e996cf74b3624e6"
)
EXTERNAL_SOURCE = Path(
    "C:/Users/Administrator/Desktop/sucai/装备/"
    "ChatGPT Image 2026年7月28日 12_58_46.png"
)
SOURCE = (
    ROOT
...
def alpha_crop(image: Image.Image, label: str) -> Image.Image:
    ...
    if box is None:
        raise ValueError(f"empty item-228 view: {label}")
...
def import_project_source() -> None:
    ...
    if not EXTERNAL_SOURCE.exists():
        raise FileNotFoundError(
            "item-228 project source is absent and the authorized external "
...
def prepare(check: bool) -> dict[str, object]:
    ...
    if not SOURCE.exists() or file_sha256(SOURCE) != SOURCE_SHA256:
        raise AssertionError("item-228 user source hash changed")
    source = Image.open(SOURCE).convert("RGBA")
    if source.size != SOURCE_SIZE:
        raise AssertionError(
            f"item-228 user source size changed: {source.size}"
        )
