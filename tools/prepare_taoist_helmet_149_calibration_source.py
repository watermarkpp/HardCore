...
SOURCE_SIZE = (1350, 1637)
SOURCE_SHA256 = (
    "27a87e2cf4e49d3e6fd093a6330f6ee55921908ed074ab486ea0a72262f41408"
)
EXTERNAL_SOURCE = Path(
    "C:/Users/Administrator/Desktop/sucai/装备/"
    "ChatGPT Image 2026年7月27日 22_27_18.png"
)
SOURCE = (
    ROOT
...
DIRECTION_ROOT = (
    ROOT
    / "assets/art/items/client/world_wear/helmet/male/source"
    / "taoist_149_directions"
)
INVENTORY_SOURCE = (
    ROOT
    / "assets/art/items/client/world_wear/helmet/male/source"
    / "taoist_149_presentation"
    / "inventory.png"
)

# The user-authored sheet already contains real alpha. These rectangles merely
# separate the two world rows and the ninth inventory-only view; no matte
# removal, face-window generation, colour change, or resampling is performed.
WORLD_ROW_BANDS = ((0, 600), (600, 1100))
INVENTORY_REGION = (0, 1100, 338, 1637)
WORLD_CELL_SIZE = (261, 401)
...
def alpha_crop(image: Image.Image, label: str) -> Image.Image:
    ...
    if box is None:
        raise ValueError(f"empty item-149 view: {label}")
...
def import_project_source() -> None:
    ...
    if not EXTERNAL_SOURCE.exists():
        raise FileNotFoundError(
            "item-149 project source is absent and the authorized external "
...
def prepare(check: bool) -> dict[str, object]:
    ...
    if not SOURCE.exists() or file_sha256(SOURCE) != SOURCE_SHA256:
        raise AssertionError("item-149 user source hash changed")
    source = Image.open(SOURCE).convert("RGBA")
    if source.size != SOURCE_SIZE:
        raise AssertionError(
            f"item-149 user source size changed: {source.size}"
        )
    ...
    sheet_width = WORLD_CELL_SIZE[0] * 4
    sheet_height = WORLD_CELL_SIZE[1] * 2
    transparent_world_sheet = Image.new(
        "RGBA", (sheet_width, sheet_height), (0, 0, 0, 0)
    )
    for slot, direction in enumerate(DIRECTIONS):
        image = direction_images[direction]
        cell_x = (slot % 4) * WORLD_CELL_SIZE[0]
        cell_y = (slot // 4) * WORLD_CELL_SIZE[1]
        destination = (
            cell_x + (WORLD_CELL_SIZE[0] - image.width) // 2,
            cell_y + (WORLD_CELL_SIZE[1] - image.height) // 2,
        )
