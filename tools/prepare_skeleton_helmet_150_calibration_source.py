...
SOURCE_SIZE = (1491, 1055)
SOURCE_SHA256 = (
    "de5753d3b36797d0937967032be238b3cde55582588ff4b99d42794740605c0d"
)
EXTERNAL_SOURCE = Path(
    "C:/Users/Administrator/Desktop/sucai/装备/"
    "ChatGPT Image 2026年7月28日 08_55_16.png"
)
SOURCE = (
    ROOT
...
def alpha_crop(image: Image.Image, label: str) -> Image.Image:
    ...
    if box is None:
        raise ValueError(f"empty item-150 view: {label}")
...
def import_project_source() -> None:
    ...
    if not EXTERNAL_SOURCE.exists():
        raise FileNotFoundError(
            "item-150 project source is absent and the authorized external "
...
def prepare(check: bool) -> dict[str, object]:
    ...
    if not SOURCE.exists() or file_sha256(SOURCE) != SOURCE_SHA256:
        raise AssertionError("item-150 user source hash changed")
    source = Image.open(SOURCE).convert("RGBA")
    if source.size != SOURCE_SIZE:
        raise AssertionError(
            f"item-150 user source size changed: {source.size}"
        )
