...
SOURCE_SIZE = (1774, 1343)
SOURCE_SHA256 = (
    "8c4990e164b528a09833b55b99d94c51f3d254ecf4bf7a3b3a71815621232063"
)
EXTERNAL_SOURCE = Path(
    "C:/Users/Administrator/Desktop/sucai/装备/"
    "ChatGPT Image 2026年7月29日 08_20_20.png"
)
SOURCE = (
    ROOT
...
DIRECTION_ROOT = (
    ROOT
    / "assets/art/items/client/world_wear/helmet/male/source"
    / "mystery_218_directions"
)
INVENTORY_SOURCE = (
    ROOT
    / "assets/art/items/client/world_wear/helmet/male/source"
    / "mystery_218_presentation"
    / "inventory.png"
)

# The user-authored sheet already contains real alpha. These bands only
# separate the two four-direction world rows from the ninth, uncut-face S view
# reserved for inventory presentation. No matte or face-window processing is
# permitted here.
...
def alpha_crop(image: Image.Image, label: str) -> Image.Image:
    ...
    if box is None:
        raise ValueError(f"empty item-218 view: {label}")
...
def import_project_source() -> None:
    ...
    if not EXTERNAL_SOURCE.exists():
        raise FileNotFoundError(
            "item-218 project source is absent and the authorized external "
...
def prepare(check: bool) -> dict[str, object]:
    ...
    if not SOURCE.exists() or file_sha256(SOURCE) != SOURCE_SHA256:
        raise AssertionError("item-218 user source hash changed")
    source = Image.open(SOURCE).convert("RGBA")
    if source.size != SOURCE_SIZE:
        raise AssertionError(
            f"item-218 user source size changed: {source.size}"
        )
